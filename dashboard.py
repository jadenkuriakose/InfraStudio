...
with st.sidebar:
    ...
    selectedDataset = st.selectbox("Dataset", datasetNames, key="ds_select")
    ...
    if selectedDataset:
        ...
        if dsInfo:
            with st.expander("📊 Dataset Info", expanded=False):
                ...
                cols = st.columns(3)
                cols[0].metric("Rows", dsInfo.get("rows"))
                cols[1].metric("Features", dsInfo.get("features"))
                cols[2].metric("Classes", dsInfo.get("classes"))
                balance = dsInfo.get("classBalance", {})
                if balance:
                    fig = px.bar(
                        x=list(balance.keys()), y=list(balance.values()),
                        labels={"x": "Class", "y": "Ratio"}, height=150,
                    )
                    fig.update_layout(margin=dict(t=10, b=10, l=10, r=10))
                    st.plotly_chart(fig, use_container_width=True)
                st.caption("Features: " + ", ".join(dsInfo.get("featureNames", [])))
    ...
    if st.button("▶ Submit Job", use_container_width=True, type="primary"):
        payload = {
            "type": jobType,
            "config": {
                "model": model, "dataset": selectedDataset,
                "epochs": epochs, "batchSize": batchSize,
                "learningRate": lr, "hiddenDims": hiddenDims, "dropout": dropout,
...
def runsDataframe(runList: list) -> pd.DataFrame:
    ...
    for r in runList:
        m = r.get("metrics", {})
        rows.append({
            "runId": r["runId"][:8], "fullId": r["runId"],
            "type": r["type"], "model": r["config"].get("model", "—"),
            "dataset": r["config"].get("dataset", "—"),
            "status": f"{statusColors.get(r['status'], '')} {r['status']}",
            "accuracy": m.get("accuracy"), "f1": m.get("f1"),
            "loss": m.get("loss"), "runtimeMs": m.get("runtimeMs"),
            "latencyMs": m.get("latencyMs"), "throughput": m.get("throughput"),
...
with tabRuns:
    ...
    if not filtered:
        ...
        st.dataframe(
            df.drop(columns=["fullId", "createdAt"]),
            use_container_width=True,
            hide_index=True,
        )
        st.caption(f"{len(filtered)} runs shown")

        if st.session_state.selectedRun:
        ...
        if not run:
            ...
            with colR:
                ...
                if m:
                    ...
                    if epochCurve:
                        fig = px.line(
                            x=list(range(1, len(epochCurve) + 1)), y=epochCurve,
                            labels={"x": "Epoch", "y": "Loss"}, title="Training Loss Curve",
                        )
                        st.plotly_chart(fig, use_container_width=True)
                    ...
                    if confMatrix:
                        ...
                        fig = px.imshow(
                            confMatrix, text_auto=True, color_continuous_scale="Blues",
                    ...
                    if featureImp:
                        ...
                        fig = px.bar(
                            x=[v for _, v in sortedFeat], y=[k for k, _ in sortedFeat],
...
with tabMetrics:
    ...
    if not completed:
        ...
        with col1:
            fig = px.bar(df, x="runId", y="accuracy", color="model",
                title="Accuracy by Run", labels={"runId": "Run", "accuracy": "Accuracy"})
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            fig = px.scatter(df, x="runtimeMs", y="accuracy", color="type",
                size="throughput", hover_data=["model", "dataset", "runId"],
        ...
        if len(compareIds) >= 2:
            ...
            for r in compareRuns:
                m = r.get("metrics", {})
                compareRows.append({
                    "runId": r["runId"][:8], "model": r["config"].get("model"),
                    "accuracy": m.get("accuracy"), "f1": m.get("f1"),
        ...
        if len(backtestRuns) > 1:
            ...
            for r in backtestRuns:
                ...
                rows.append({
                    "runId": r["runId"][:8], "model": r["config"].get("model"),
                    "dataset": r["config"].get("dataset"),
                    "meanAccuracy": m.get("accuracy"), "accStd": m.get("loss"),
                    "meanF1": m.get("f1"), "runtimeMs": m.get("runtimeMs"),
                })
            fig = px.scatter(pd.DataFrame(rows), x="meanAccuracy", y="accStd",
                color="model", size="runtimeMs", hover_data=["runId", "dataset"],
                title="Accuracy vs Stability (lower std = more stable)")
            st.plotly_chart(fig, use_container_width=True)

with tabDatasets:
    if not datasets:
        st.error("Could not reach orchestrator")
    else:
        for d in datasets:
            with st.expander(f"📂 {d['name']}", expanded=False):
                st.caption(d.get("description", ""))
                cols = st.columns(4)
                cols[0].metric("Rows", d.get("rows"))
                cols[1].metric("Features", d.get("features"))
                cols[2].metric("Classes", d.get("classes"))
                balance = d.get("classBalance", {})
                cols[3].metric("Class Ratio", " : ".join(f"{v:.0%}" for v in balance.values()))

                colL, colR = st.columns(2)
                with colL:
                    if balance:
                        fig = px.pie(values=list(balance.values()), names=list(balance.keys()),
                            title="Class Distribution", height=250)
                        st.plotly_chart(fig, use_container_width=True)
                with colR:
                ...
                runsOnDataset = [r for r in runs if r["config"].get("dataset") == d["name"] and r["status"] == "completed"]
                if runsOnDataset:
                    ...
                    bestModel = next((r["config"].get("model") for r in runsOnDataset
